...
do {
    let player = try myTeam.getPlayerByNumber(3)
    print("Player is \(player.firstName) \(player.lastName)")
} catch let error {
    print("Error:  \(error)")
}

if let player = try? myTeam.getPlayerByNumber(3) {
    print(player.firstName )
}

if let nothing = try? myTeam.addPlayer(("X", "Man", 1)) {}

do {
    try myTeam.addPlayer(("Derek", "Jeter", 2))
} catch PlayerNumberError.NumberTooHigh(let description) {
    print("Error: \(description)")
} catch PlayerNumberError.NumberTooLow(let description) {
    print("Error: \(description)")
