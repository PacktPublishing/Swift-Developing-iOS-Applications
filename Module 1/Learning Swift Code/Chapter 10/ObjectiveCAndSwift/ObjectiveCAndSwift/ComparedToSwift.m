...
//: unwrapping
int* variableNumber3 = nil;
int actualNumber = *variableNumber2;
...
[array4 addObject:@"Hello World!"];

//: Accessing an object
NSString *myString2 = [array objectAtIndex:0];
NSString *myString3 = array[0];
NSArray *myString4 = array[0];
...
//: Accessing an object
NSString *myString5 = [dict objectForKey:@"Hello"];
NSString *myString6 = dict[@"Hello"];
...
NSArray *addInviteeToListIfSpotAvailable2
(
    NSArray *invitees,
    NSString *newInvitee
)
...
void main3() {
    NSArray *invitees = @[@"Sarah"];
    addInviteeToListIfSpotAvailable2(invitees, @"Roana");
}
...
void main6() {
    [Contact2 printAvailablePhonePrefixes];
}
...
//    }
//}
@interface Contact2 (Helpers)
- (NSString *)fullName;
@end

@implementation Contact2 (Helpers)
- (NSString *)fullName {
...
@implementation Contact2 (Extra)
//: Defining a method that takes a closure
- (id)firstInArray:(NSArray *)array passingTest:(BOOL(^)(id))test
{
    for (id element in array) {
        if (test(element)) {
            return element;
...
void main9() {
    Ball2 *ball = [Ball2 new];
    __weak Ball2 *weakBall = ball;
    ball.onBounce = ^{
...
@interface SteeringWheel2 : NSObject
@property (weak) Car *car;
@end
